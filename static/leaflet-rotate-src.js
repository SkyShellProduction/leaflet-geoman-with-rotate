...
(function () {
  ...
  L.extend(L.DomUtil, {
     ...
     * @param {L.Point} pivot
     */
    setTransform(el, offset, scale, bearing, pivot) {
      ...
      el.style[L.DomUtil.TRANSFORM] = `translate3d(${pos.x}px, ${pos.y}px, 0) ${
        scale ? `scale(${scale})` : ``
      } rotate(${bearing}rad)`;
      return undefined;
    },
     ...
     * @param {Number} scale
     */
    setPosition(el, point, bearing, pivot, scale) {
      // if (mapPm) {
      //   mapPm.pm.Draw._unsnap();
      // }
      if (!bearing) {
  ...
  L.DivOverlay.include({
    ...
    _updatePosition() {
      ...
      if (this._map && this._map._rotate && this._zoomAnimated) {
        ...
        const pos = L.DomUtil.getPosition(this._container).subtract(anchor);
  ...
  L.Popup.include({
    ...
    _adjustPan: function () {
      ...
      let containerPos = layerPos._add(this._map._getMapPanePos()),
  ...
  let markerDragProto; // retrived at runtime (see below: L.Marker::_initInteraction())
  ...
  const MarkerDrag = {
    ...
    _onDrag(e) {
      ...
      /** @TODO use markerDragProto._onDrag */
      if (rotatedMarker) marker.setLatLng(latlng);
  ...
  L.Marker.include({
    ...
    _initInteraction() {
      const ret = markerProto._initInteraction.apply(this, arguments);
      if (
        this.dragging &&
    ...
    _setPos(pos) {
      ...
      /** @TODO use markerProto._setPos */
      let bearing = this.options.rotation || 0;
  ...
  L.GridLayer.include({
     ...
     */
    getEvents() {
      const events = gridLayerProto.getEvents.apply(this, arguments);
  ...
  L.Map.include({
    ...
    //     const projectedPoint = this.project(L.latLng(latlng))._round();
     ...
     */
    mapBoundsToContainerBounds(bounds) {
      ...
      const ne = this.layerPointToContainerPoint(
        this.project(bounds.getNorthEast())._subtract(origin)
      );
      const sw = this.layerPointToContainerPoint(
        this.project(bounds.getSouthWest())._subtract(origin)
      );
      const se = this.layerPointToContainerPoint(
    ...
    setBearing(theta) {
      ...
      const bearing = L.Util.wrapNum(theta, [0, 360]) * L.DomUtil.DEG_TO_RAD;
      const center = this._getPixelCenter();
      const oldPos = this._getRotatePanePos().rotateFrom(
        -this._bearing,
        center
      );
      ...
      this._bearingClone =
        bearing !== 0 ? radToDeg(bearing) : this._bearingClone;
    ...
    getBearingClone() {
      return this._bearingClone;
    },
    setBearingClone(int) {
      this._bearingClone = int;
    },
    ...
    panInside(latlng, options) {
      ...
      const pixelPoint = this.latLngToContainerPoint(latlng);
    ...
    getBoundsZoom(bounds, inside, padding) {
      ...
      const boundsSize = this.mapBoundsToContainerBounds(bounds).getSize();
     ...
     */
    _getNewPixelBounds(center, zoom) {
      ...
      const mapZoom = this._animatingZoom
        ? Math.max(this._animateToZoom, this.getZoom())
        : this.getZoom();
      const scale = this.getZoomScale(mapZoom, zoom);
      const pixelCenter = this.project(center, zoom).floor();
      const size = this.getSize();
    ...
    //     const minX = points.reduce(function (pv, v) { return Math.min(pv, v.x); }, points[0].x),
  ...
  L.Map.ContainerMutation = L.Handler.extend({
    addHooks() {
      ...
      const observeData = L.Util.bind(this._map.invalidateSize, this._map);
      const mapPm = this._map;
      if (!this._observer) {
        this._observer = new MutationObserver((observeData) => {
          if (mapPm.getBearing() === 0) {
            mapPm.setBearing(mapPm.getBearingClone());
          }
        });
      }
      ...
      if (this._zooming) {
        ...
        if (map.options.touchZoom === "center") {
          ...
          this._center = map.unproject(
            map.project(this._pinchStartLatLng).subtract(delta.rotate(alpha))
          );
        }
      }
  ...
  L.Map.ShiftKeyRotate = L.Handler.extend({
    ...
    _handleShiftScroll(e) {
      if (e.shiftKey) {
        e.preventDefault();
        this._map.scrollWheelZoom.disable();
        this._map.setBearing(
          this._map._bearing * L.DomUtil.RAD_TO_DEG + Math.sign(e.deltaY) * 5
        );
      } else {
  ...
  L.Control.Rotate = L.Control.extend({
    ...
    onAdd(map) {
      ...
      const arrow = (this._arrow = L.DomUtil.create(
        "span",
      ...
      const link = (this._link = L.DomUtil.create(
        "a",
    ...
    _restyle() {
      if (!this._map.options.rotate) {
        ...
        // Locked mode
          ...
          if (bearing === 0 && this.options.closeOnZeroBearing) {
